...
const getAuth = async (req, res, next) => {
  try {
    ...
    // console.log('resService:', resServices)
    return controller.sendSuccess(res, resServices.data, 200, resServices.message)
  } catch (error) { // bug
    console.log(error)
    return controller.sendError(res)
...
const updateProfile = async (req, res, next) => {
  try {
    ...
    const { fullName, address, avatar, phoneNumber } = req.body
    console.log("controller", id)
    const resServices = await authServices.updateUserProfile(id, { fullName, address, avatar, phoneNumber })
    console.log('controller', resServices)
    if (!resServices.success) {
      return controller.sendSuccess(res, resServices.success, 300, resServices.message)
    }
